...
/* =================================================================
   RUST STANDALONE HTML EXPORT ENGINE & MODAL
================================================================== */
...
function buildFullStandaloneHTML(payload) {
  ...
  const custom_todos_str = JSON.stringify(custom_todos || []);
  const calendar_events_str = JSON.stringify(calendar_events || []);
  ...
  const tree_heading = is_ar ? "خريطة المفاهيم" : "Knowledge Tree Map";
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .plate { background: var(--PageBG); color: var(--BodyText); width: 210mm; min-height: 297mm; max-width: 100%; box-sizing: border-box; padding: 16mm 14mm 20mm; margin: 24px auto; font-family: 'Amiri', 'Noto Naskh Arabic', 'Cairo', serif; font-size: 17px; line-height: 1.85; box-shadow: 0 10px 32px rgba(0,0,0,0.12), 0 2px 6px rgba(0,0,0,0.05); border-radius: 4px; position: relative; }
  ...
  .opt-btn { display: block; width: 100%; text-align: start; border: 1.5px solid rgba(0,0,0,0.15); background: rgba(255,255,255,0.75); color: inherit; border-radius: 12px; padding: 10px 14px; margin-bottom: 8px; cursor: pointer; font-size: 0.95rem; font-weight: 600; transition: all 0.18s; }
  ...
  .btn-check { background: rgba(20,21,26,0.92); color: #fff; border: none; padding: 9px 20px; border-radius: 999px; font-weight: 700; font-size: 0.85rem; cursor: pointer; transition: transform 0.15s; margin-top: 6px; }
...
async function exportStandalonePackage({ targetType, targetId, books, bags, lang, covers }) {
  ...
  if (targetType === "bag") {
    ...
    tagline = `Bag Collection (${bagBooks.length} Books / Encyclopedias)`;
    cover_icon = bag?.icon || "briefcase";
  ...
  // Attempt Rust Tauri IPC compilation first
  try {
    if (window.__TAURI_INTERNALS__ || window.__TAURI__) {
      const { invoke } = await import("@tauri-apps/api/core");
      compiledHtml = await invoke("export_standalone_bundle", { payload });
    }
  } catch (err) {
    console.info("Tauri native IPC not active, using JS compiler", err);
  }

  if (!compiledHtml) {
    compiledHtml = buildFullStandaloneHTML(payload);
  }
