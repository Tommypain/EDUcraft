...
/* =================================================================
   STANDALONE HTML EXPORT ENGINE (REAL APP-BUNDLE INLINER)
================================================================== */
...
async function buildAppBundleFallback(payload) {
  const { target_type, title, lang, dir, cover_from, cover_to, data } = payload;
  const from = cover_from || "#E8654A";
  ...
  for (const sc of scriptEls) {
    if (sc.src && !sc.src.includes("@vite/client")) {
      try {
        const res = await fetch(sc.src);
        if (res.ok) {
          const js = await res.text();
          scriptsHtml += `<script type="module">${js}</script>\n`;
        }
      } catch (e) {
        scriptsHtml += `<script type="module" src="${sc.src}"></script>\n`;
      }
    } else if (sc.innerHTML && !sc.innerHTML.includes("window.__STANDALONE__")) {
